...
subroutine mass_constant_temperature_newtonian(central_density,temperature, &
  ...
  flag_return = 0
  do i=2,N
     call dpdr(TOVmass(i-1),TOVpressure(i-1),TOVrho(i-1),TOVrad(i-1),k1p)
     call dmdr(TOVpressure(i-1),TOVrho(i-1),TOVrad(i-1),k1m)
     call dphidr(TOVmass(i-1),TOVpressure(i-1),TOVrho(i-1),TOVrad(i-1),k1ph)
     ...
     call dmdr(TOVpressure(i-1)+0.5d0*dr*k1p,temp_rho,TOVrad(i-1)+0.5d0*dr,k2m)
     ...
     call dmdr(TOVpressure(i-1)+dr*0.5d0*k2p,temp_rho,TOVrad(i-1)+0.5d0*dr,k3m)
     ...
     call dmdr(TOVpressure(i-1)+dr*k3p,temp_rho,TOVrad(i-1)+dr,k4m)
